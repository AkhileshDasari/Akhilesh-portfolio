...
export function IconCloud({ iconSlugs, images }: DynamicCloudProps) {
  ...
  if (!mounted) {
    return <div className="flex justify-center items-center w-full min-h-[400px]" />;
  }

  return (
    <Cloud id="icon-cloud-container" {...cloudProps}>
      {renderedIcons}
      {images &&
        images.map((img) => (
          <a key={img} href="#" onClick={(e) => e.preventDefault()}>
            <img src={img} alt="icon" width={42} height={42} />
          </a>
        ))}
    </Cloud>
  );
